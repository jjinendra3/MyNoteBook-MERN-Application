...
router.post(
  "/createuser",
  [
    body("name", "Enter a valid name of minimum 5 characters.").isLength({
      min: 5,
    }), //express validator checks the functions given
    body("email", "The format should be that of an email.").isEmail(),
    body(
      "password",
      "Enter a valid password of minimum 5 characters."
    ).isLength({ min: 5 }),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const data = {
        //this is used for jwt
        //this is the user variable that is created to store the variables
        user: {
